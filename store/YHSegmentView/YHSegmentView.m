...
- (UIScrollView *)segmentContentView {
    if (!_segmentContentView) {
       ...
       // for (int i=0; i<self.viewControllersArr.count; i++) {
            // 默认加载第一个控制器
        if (self.viewControllersArr.count) {
            self.bottomLineView.hidden = NO;
            categoryGoodsViewController *viewController = self.viewControllersArr[0];
            viewController.subClassBlock = ^(id  _Nonnull object) {
                if (self.contentBlock) {
                    self.contentBlock(object);
                }
            };
            ...
            [viewController reloadDataWithDataArr:@[@"1"]];
            [_segmentContentView addSubview:viewController.view];
        } else {
            self.bottomLineView.hidden = YES;
//            [MBProgressHUD showToastText:@"该类目下暂无内容" toView:self];
        }
...
- (UIView *)indicateView {
    if (!_indicateView) {
        ...
        _indicateView.backgroundColor = myBlueBg;
...
- (instancetype)initWithFrame:(CGRect)frame ViewControllersArr:(NSArray *)viewControllersArr TitleArr:(NSArray *)titleArr TitleNormalSize:(CGFloat)titleNormalSize TitleSelectedSize:(CGFloat)titleSelectedSize SegmentStyle:(YHSegementStyle)style ParentViewController:(UIViewController *)parentViewController ReturnIndexBlock:(yh_indexBlock)indexBlock ReturnContentClickBlock:(contentClickBlock)contentBlock{
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [UIColor groupTableViewBackgroundColor];
//        UILabel *blankLb = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, 20)];
//        blankLb.center = CGPointMake(self.center.x, self.center.y-120);
//        blankLb.text = @"改类目下暂无内容";
//        blankLb.textColor = RGB(100, 100, 100);
//        blankLb.font = [UIFont systemFontOfSize:15];
//        [self addSubview:blankLb];
...
- (void)loadOtherVCWith:(NSInteger)tag {
    ...
    categoryGoodsViewController *viewController = self.viewControllersArr[tag];
