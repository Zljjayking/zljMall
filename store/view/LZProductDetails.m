...
- (ZGLVideoPlyer *)player {
    if (!_player) {
        _player = [[ZGLVideoPlyer alloc]initWithFrame:CGRectMake(0, 0, self.videoCoverImgV.frame.size.width, self.videoCoverImgV.frame.size.height)];
    }
    return _player;
}
- (UIButton *)ToVideoBtn{
...
- (void)updateUIWithImageAndVideoArray:(NSArray *)detailsArr vipSignImage:(UIImageView *)signImage{
    ...
    self.scrolV.contentSize = CGSizeMake(detailsArr.count*self.frame.size.width, 0);
    ...
    for (int i = 0; i < detailsArr.count; i ++) {
        ...
        if ([self.isHaveVideo isEqualToString:@"1"]) {
            if (i == 0) {
                ...
                self.player.placeHolderStr = self.goodsPreview;
                self.player.videoUrlStr = detailsArr[0];
                [self.videoCoverImgV addSubview:self.player];
                ...
                NSString *imageUrl = detailsArr[i];
                [imgV sd_setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:[UIImage imageNamed:@"goodss"] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                    [MBProgressHUD hideHUD];
                }];
                if (signImage) {
                    if (i == 1) {
                        [imgV addSubview:signImage];
                    }
                }
            ...
            imgV.clipsToBounds = YES;
            NSString *imageUrl = detailsArr[i];
            [imgV sd_setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:[UIImage imageNamed:@"goods"]];
            [self.scrolV addSubview:imgV];
        }
    ...
    if ([self.isHaveVideo isEqualToString:@"1"]) {
        ...
        self.indexLab.frame = CGRectMake(self.frame.size.width - 40 - 20, self.frame.size.height - 25 - 20, 40, 25);
        self.indexLab.center = CGPointMake(self.center.x, self.frame.size.height - 12.5 - 20);
        self.indexLab.text = [NSString stringWithFormat:@"%d/%d",1,(int)detailsArr.count];
        [self addSubview:self.indexLab];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if ([self.isHaveVideo isEqualToString:@"1"]) {
        ...
        self.indexLab.text = [NSString stringWithFormat:@"%d/%d",(int)index+1,(int)self.productDetailsArr.count];
    }
