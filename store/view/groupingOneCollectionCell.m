...
@implementation groupingOneCollectionCell

- (void)awakeFromNib {
    [super awakeFromNib];
    self.titleLb.text = @"正在进行的拼团";
    self.titleLb.textColor = myRed;
    self.titleLb.font = [UIFont boldSystemFontOfSize:15];
    self.headImage.layer.cornerRadius = 20;
    [self.findMoreBtn addTarget:self action:@selector(findMoreBtnClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setSpellmodel:(spellModel *)spellmodel {
    ...
    if (spellmodel.tel.length == 11) {
        self.moblieLb.text = [spellmodel.tel stringByReplacingCharactersInRange:NSMakeRange(3, 4) withString:@"****"];
    }else {
        self.moblieLb.text = spellmodel.tel;
    }
    ...
    dispatch_source_set_event_handler(self.timer, ^{
        ...
        if (time >= 0) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                weakSelf.poorTimeLb.text = [NSString stringWithFormat:@"剩余%d:%02d:%02d.%d",h,m,s,ms];
            });
        }else {
